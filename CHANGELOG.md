...
### Changed
- **Linting consolidated onto ruff** - The docs told you to run `ruff check shuffify/` while CI ran `flake8`, and ruff was in no requirements file. Adopted ruff for real: config moves from `.flake8` (deleted) into `pyproject.toml`, the repo's first. `select = ["E", "W", "F", "I"]` reproduces what flake8 enforced and adds import sorting, which replaces the `isort` dev dependency that was installed but wired into neither CI nor any documented command. Verified parity before switching: ruff's E/W/F finds zero issues on `shuffify/`, exactly as flake8 did. `mypy` stays -- it is a type checker, not a linter, and ruff does not subsume it. CI, `CLAUDE.md` (which documented both commands, in the same file) and the dependabot dev-dependency group updated together. `black` remains non-blocking: SR-037 consolidates *linting*, not formatting, and its 53-file drift is pre-existing. Closes #458 (SR-037)
